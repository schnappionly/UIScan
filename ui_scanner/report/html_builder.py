...
.element-item { margin-left: 24px; padding: 4px 0; font-size: 13px; color: #555; display: flex; gap: 8px; align-items: baseline; }
...
function applyFilters() {
    const items = document.querySelectorAll('[data-platform]');
    let visible = 0;
    items.forEach(el => {
        ...
        if (filterCategory !== 'all' && cat !== filterCategory) show = false;
        if (searchText && !text.toLowerCase().includes(searchText.toLowerCase())) show = false;
...
class HTMLBuilder:
    ...
    def _build_stats(self, buf: StringIO):
        r = self.result
        android_pages = sum(len(p.elements) for m in r.android_modules for p in m.pages)
        ios_pages = sum(len(p.elements) for m in r.ios_modules for p in m.pages)
    ...
    def _build_controls(self, buf: StringIO):
        ...
        buf.write('<input type="text" id="search" placeholder="搜索元素名称、类型、ID...">')
    ...
    def _build_content(self, buf: StringIO):
        ...
        for platform in [Platform.ANDROID, Platform.IOS]:
            modules = self.result.android_modules if platform == Platform.ANDROID else self.result.ios_modules
            if not modules:
                continue
            plat_label = platform.value
            icon = '🤖' if platform == Platform.ANDROID else '🍎'
    ...
    def _build_module(self, buf: StringIO, module: ModuleInfo, platform: str):
        buf.write(f'<details class="module-section" data-platform="{platform}">')
    ...
    def _build_page(self, buf: StringIO, page: Page, platform: str):
        ...
        for cat_type in [ElementType.CLICKABLE, ElementType.DISPLAY, ElementType.ANIMATION,
            ...
            buf.write(f'data-category="{cat_type.value}">')
            elem_texts = ' '.join(f'{e.name} {e.element_id} {e.element_type}' for e in elems)
            buf.write(f'data-text="{_esc_attr(search_text + " " + elem_texts)}">')
            buf.write(f'<summary class="category-header {cat_class}">')
            buf.write(f'{cat_type.value} ({len(elems)})</summary>')

            for elem in elems:
                self._build_element(buf, elem)
        ...
        if page.dialogs:
            ...
            dialog_text = ' '.join(d.name + d.scenario for d in page.dialogs)
            buf.write(f'data-text="{_esc_attr(search_text + " dialog " + dialog_text)}">')
            buf.write(f'<summary class="category-header dialog-header">弹框 ({len(page.dialogs)})</summary>')
            for dialog in page.dialogs:
                self._build_dialog(buf, dialog)
            buf.write('</details>')

        buf.write('</details>')

    def _build_element(self, buf: StringIO, elem: UIElement):
        buf.write('<div class="element-item">')
        ...
        if elem.line_number:
            buf.write(f'<span class="element-props">L{elem.line_number}</span>')
        buf.write('</div>')

    def _build_dialog(self, buf: StringIO, dialog: DialogInfo):
        buf.write('<div class="element-item">')
        ...
        if dialog.line_number:
            buf.write(f'<span class="element-props">L{dialog.line_number}</span>')
        buf.write('</div>')
